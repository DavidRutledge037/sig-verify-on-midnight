...
import { IMidnightService } from '../types/services';
import { TransactionDetails, Block, ProofSubmission, TransactionResponse } from '../types/transaction';
...
export class MidnightService implements IMidnightService {
    ...
    async submitProof(proof: ProofSubmission): Promise<string> {
        try {
            const response = await fetch(`${this.config.nodeUrl}/submit_proof`, {
                method: 'POST',
                headers: {
    ...
    async verifyProof(txHash: string): Promise<boolean> {
        try {
            const response = await fetch(`${this.config.nodeUrl}/verify_proof/${txHash}`);
            if (!response.ok) {
                throw new Error(`API request failed: ${response.status} ${response.statusText}`);
            }
    ...
    async getTx(hash: string): Promise<Uint8Array> {
        try {
            const response = await fetch(`${this.config.nodeUrl}/tx_raw/${hash}`);
            if (!response.ok) {
                throw new Error(`API request failed: ${response.status} ${response.statusText}`);
            }
    ...
    async getBlock(height: number): Promise<any> {
        try {
            const response = await fetch(`${this.config.nodeUrl}/block/${height}`);
            if (!response.ok) {
                throw new Error(`API request failed: ${response.status} ${response.statusText}`);
            }
